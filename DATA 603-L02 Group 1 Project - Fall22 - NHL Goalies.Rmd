...
# Do winning % and goalie save % have a relationship?, in other words, why are we doing this?
```{r}
...
```{r}
ggplot(teams, aes(x = Save, y = Points)) +
  geom_point(size = 1)
```

$$
Points\ Percentage = 4.407Save\ Percentage - 3.449
$$
...
ggplot(data=teams,mapping= aes(x=Save,y=Points))+geom_point()+
stat_function(fun=teamfunction,geom="line")+ggtitle("Team Points Percentage vs. Team Save Percentage (1997/98-2018/19)")+labs(y= "Team Points Percentage", x = "Team Save Percentage")
```

# Main dataset model build and analysis
```{r}
